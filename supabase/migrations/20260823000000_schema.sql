...
-- Schema for moving the books off one browser and onto a shared database.
--
...
create or replace function next_no(p_kind text, p_prefix text) returns text
  language plpgsql security definer set search_path = public as $$
declare n integer;
begin
  insert into doc_seq (kind, value) values (p_kind, 1)
    on conflict (kind) do update set value = doc_seq.value + 1
...
create index if not exists courses_center_idx on courses (upper(center));
create index if not exists courses_title_idx  on courses (upper(title));
...
create index if not exists trainees_name_idx on trainees (upper(last), upper(first));
create index if not exists trainees_srn_idx  on trainees (srn) where srn <> '';

-- ---------------------------------------------------------------- bookings
create table if not exists enrollments (
  id                text primary key,
  no                text unique not null,
  trainee_id        text not null references trainees (id) on delete restrict,
...
create index if not exists enrollments_trainee_idx on enrollments (trainee_id);
create index if not exists enrollments_center_idx  on enrollments (upper(center));
create index if not exists enrollments_open_idx    on enrollments (center) where remit_no is null;
...
create index if not exists invoices_trainee_idx on invoices (trainee_id);
...
create index if not exists payments_invoice_idx on payments (invoice_id) where not voided;
...
create or replace view invoice_status as
  select i.id,
    ...
    from invoices i
    left join (
      select invoice_id, sum(amount) as paid from payments where not voided group by invoice_id
...
create index if not exists journal_ref_idx  on journal (ref_id);
...
begin
  ...
  loop
    ...
    execute format('create policy %I on %I for insert with check (is_staff())', t || '_add', t);
  end loop;
end $$;

-- Updating is narrower than reading.
create policy trainees_edit    on trainees    for update using (is_staff());
create policy enrollments_edit on enrollments for update using (is_staff());
create policy invoices_edit    on invoices    for update using (is_staff());
create policy payments_edit    on payments    for update using (is_staff());
create policy expenses_edit    on expenses    for update using (is_staff());
create policy refunds_edit     on refunds     for update using (is_staff());

-- The price list, the chart of accounts and the office profile are the admin's.
create policy courses_edit  on courses  for update using (is_admin());
create policy courses_drop  on courses  for delete using (is_admin());
create policy accounts_edit on accounts for update using (is_admin());
create policy company_edit  on company  for update using (is_admin());
...
-- No update policy on journal, and no delete policy anywhere. Postgres denies
-- what is not allowed, so the ledger can be added to and never altered, and
-- nothing at all can be deleted through the API.

-- Anyone may register. Nobody anonymous may read what anyone else submitted.
create policy registrations_submit on registrations for insert to anon, authenticated with check (true);
create policy registrations_read   on registrations for select using (is_staff());
create policy registrations_edit   on registrations for update using (is_staff());

-- Numbering is callable by staff only.
revoke all on function next_no(text, text) from public, anon;
grant execute on function next_no(text, text) to authenticated;
